    ...
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbarCollapse">
        <ul class="navbar-nav me-auto mb-2 mb-md-0">
          ...
          <li class="nav-item">
            <div class="dropdown">
              <a class="nav-link text-white dropdown-toggle" id="dropdownMenuButton2" data-bs-toggle="dropdown" ><i class="fas fa-motorcycle"></i> Phương tiện</a>
              {{-- <ul class="dropdown-menu dropdown-menu-dark" aria-labelledby="dropdownMenuButton2" style="background: #1B4A7A;">
                ...
                <ul class="dropdown-menu dropdown-menu-light" aria-labelledby="dropdownMenuButton1" style="background: #ffffff;">
                  @foreach($categoryCar as $categoryCarParent)
                  <li><a class="dropdown-item" href="{{route('danhmucxe.index',['id'=>$categoryCarParent->id,'slug'=>$categoryCarParent->slug])}}"><Strong>{{$categoryCarParent->name}}</Strong></a>
                    @if($categoryCarParent->CategoriesCarChildrent->count())
                    <ul class="navbar-nav me-auto mb-2 mb-md-0">
                      @foreach($categoryCarParent->CategoriesCarChildrent as $CategoriescarChil)
                      <li class="nav-item">                      
                          <a class="dropdown-item" href="{{route('danhmucxe.index',['id'=>$CategoriescarChil->id,'slug'=>$CategoriescarChil->slug])}}">{{$CategoriescarChil->name}}</a>                     
                      </li>
                      @endforeach
                    </ul>
                    @endif
