      ...
      <div class="col-md-4">
        <div class="mt-5 row">
          ...
          @foreach($blogNew as $blogNew)
            <a href="{{route('chitiettintuc.index',['id'=>$blogNew->id])}}" class="text-decoration-none" id="tin">{{$blogNew->title}}</a>
          @endforeach
          <h5 class="text-warning col-10 boder-right-warning">Tin tức ngẫu nhiên</h5>
          @foreach($blogRamdom as $blogRamdom)
            <a href="{{route('chitiettintuc.index',['id'=>$blogRamdom->id])}}" class="text-decoration-none" id="tin">{{$blogRamdom->title}}</a>
          @endforeach
