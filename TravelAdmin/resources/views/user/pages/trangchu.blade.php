    ...
    <div class="container marketing">
        ...
        <div class="row ">
            ...
            <div class="col-lg-4">
                ...
                <p class="text-dark">Chấp nhận mọi hình thức thanh toán, không cần thẻ tín dụng. Bảo mật thông tin tuyệt
                    đối.</p>
            </div><!-- /.col-lg-4 -->
            <div class="col-lg-4">
                ...
                <div class="card-deck center row justify-content-center">
                    @foreach ($tours as $tour)
                        <div class="card col-5 m-3 p-0">
                            <img class="card-img-top" src="{{ $tour->feature_image_path }}" alt="Card image cap"
                                style="width: 100%; height: 100%;">
                            <div class="card-body">
                                <h5 class="card-title">{{ $tour->name }}</h5>
                                <p class="card-text m-0"><i class="fas fa-barcode"></i> <strong>Mã
                                        Tour:</strong>{{ $tour->id }}</p>
                                ...
                                <p class="card-text m-0"><i class="fas fa-map-marker-alt"></i> <strong>Nơi Khởi
                                        Hành:</strong> </p>
                                <p class="card-text m-0"><i class="fas fa-money-bill-alt"></i>
                                    <strong>Giá:</strong>{{ $tour->price }}</p>
                                <p class="card-text m-0"><i class="fas fa-calendar-alt"></i> <strong> Số Ngày:</strong>
                                    {{ $tour->duration }}</p>
                                <a href="{{ route('chitiettour.index', ['id' => $tour->id]) }}" type="button"
                                    class="btn btn-info m-2 text-white">Xem chi tiết</a>
                                <a href="{{ route('booktour.index', ['id' => $tour->id]) }}" type="button"
                                    class="btn btn-info m-2 text-white">Đặt tour</a>
                            </div>
                    ...
                    @foreach ($blogs as $blog)
                        <div class="card mb-3 p-0 m-0" style="max-width: 100%   ;">
                            <div class="row g-0 col-12">
                                <div class="col-md-4">
                                    <a href="{{ route('chitiettintuc.index', ['id' => $blog->id]) }}"><img
                                            src="{{ $blog->feature_image_path }}" class="rounded-start p-0 m-0"
                                            style="width: 100%; height: 100%;"></a>
                                </div>
                                <div class="col-md-8">
                                    <div class="card-body">
                                        <a href="{{ route('chitiettintuc.index', ['id' => $blog->id]) }}"
